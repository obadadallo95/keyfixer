...
describe('KeyFixer Website Public Legal Routes (TASK 8D)', () => {
  ...
  it('/terms loads Terms of Use correctly in English and Arabic', () => {
    ...
    expect(screen.getByText(/شراء لمرة واحدة بدون أي اشتراكات دورية/i)).toBeDefined();
  ...
  it('/refund loads Purchase & Refund Policy correctly in English and Arabic', () => {
    ...
    // Toggle to Arabic
    fireEvent.click(screen.getByTitle('Toggle Language'));
    expect(screen.getAllByText(/سياسة الشراء والاسترجاع/i).length).toBeGreaterThanOrEqual(1);
    expect(screen.getByText(/تتولى منصة المتجر المعنية معالجة طلبات الاسترداد/i)).toBeDefined();
  ...
  it('/accessibility loads Accessibility & Permission Disclosure correctly', () => {
    ...
    // Toggle to Arabic
    fireEvent.click(screen.getByTitle('Toggle Language'));
    expect(screen.getAllByText('الأذونات والوصول').length).toBeGreaterThanOrEqual(1);
    expect(screen.getByText(/نسخة Mac App Store: بدون أي إذن لتسهيلات الاستخدام/i)).toBeDefined();
  });
